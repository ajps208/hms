<?php
    ...
    if(isset($_POST['submit'])){
        ...
        $roomno=$_POST['rmno'];
        $type=$_POST['rtype'];
        $sql="SELECT bed_no FROM rooms where bed_no=?";
        $stmt1 = $mysqli->prepare($sql);
        $stmt1->bind_param('i',$bed);
        $stmt1->execute();
        $stmt1->store_result(); 
        $row_cnt=$stmt1->num_rows;;
        if($row_cnt>0){
            echo"<script>alert('Bed already exist!');</script>";
        } else {
            $query="INSERT into  rooms (bed_no,room_no,room_type) values(?,?,?)";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('iis',$bed,$roomno,$type);
            $stmt->execute();
                                ...
                                <div class="card-body">
                                    <h4 class="card-title">Bed Number</h4>
                                        <div class="form-group">
                                            <input type="text" name="bedno" placeholder="Enter Room Number" id="bedno" class="form-control" required>
                                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Room Type</h4>
                                        <div class="form-group">
                                        <select class="custom-select mr-sm-2" id="rtype" name="rtype">
